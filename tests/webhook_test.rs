...
async fn test_webhook_client_enabled() {
    let mut config = HashMap::new();
    config.insert("governance.webhook_url".to_string(), "http://localhost:8080/webhook".to_string());
    ...
    let client = GovernanceWebhookClient::new(&ctx).await.unwrap();
    assert!(client.enabled);
    assert_eq!(client.webhook_url.as_ref().unwrap(), "http://localhost:8080/webhook");
