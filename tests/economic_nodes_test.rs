...
impl NodeAPI for MockNodeAPI {
    ...
    async fn get_block(&self, _: &Hash) -> Result<Option<bllvm_protocol::Block>, bllvm_node::module::traits::ModuleError> { Ok(None) }
    async fn get_block_header(&self, _: &Hash) -> Result<Option<bllvm_protocol::BlockHeader>, bllvm_node::module::traits::ModuleError> { Ok(None) }
    async fn get_transaction(&self, _: &Hash) -> Result<Option<bllvm_protocol::Transaction>, bllvm_node::module::traits::ModuleError> { Ok(None) }
    async fn has_transaction(&self, _: &Hash) -> Result<bool, bllvm_node::module::traits::ModuleError> { Ok(false) }
    async fn get_chain_tip(&self) -> Result<Hash, bllvm_node::module::traits::ModuleError> { Ok([0u8; 32]) }
    ...
    async fn subscribe_events(&self, _: Vec<EventType>) -> Result<tokio::sync::mpsc::Receiver<ModuleMessage>, bllvm_node::module::traits::ModuleError> {
        let (_tx, rx) = tokio::sync::mpsc::channel(100);
        Ok(rx)
    }
    async fn get_mempool_transactions(&self) -> Result<Vec<Hash>, bllvm_node::module::traits::ModuleError> { Ok(Vec::new()) }
    async fn get_mempool_transaction(&self, _: &Hash) -> Result<Option<bllvm_protocol::Transaction>, bllvm_node::module::traits::ModuleError> { Ok(None) }
    ...
    async fn get_chain_info(&self) -> Result<bllvm_node::module::traits::ChainInfo, bllvm_node::module::traits::ModuleError> {
        Ok(bllvm_node::module::traits::ChainInfo { tip: [0u8; 32], height: self.block_height, difficulty: 1.0 })
    }
    ...
    async fn check_transaction_in_mempool(&self, _: &Hash) -> Result<bool, bllvm_node::module::traits::ModuleError> { Ok(false) }
    ...
    async fn publish_event(&self, _: EventType, _: EventPayload) -> Result<(), bllvm_node::module::traits::ModuleError> { Ok(()) }
...
async fn test_economic_node_registration() {
    ...
    let registry = EconomicNodeRegistry::new(&ctx, node_api.clone()).await.unwrap();
    ...
    registry.handle_event(&event, node_api.as_ref()).await.unwrap();
